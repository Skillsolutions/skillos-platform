...
import { PlusCircle, Edit, Trash2, Search, Users, BookOpen, Lightbulb, Briefcase } from 'lucide-react';
...
const JobRoleManager = () => {
  ...
  useEffect(() => {
    if (skillToTag) {
      const relevantCourses = courses.filter(course => 
        ...
        course.relatedCompetencies.some(compID => {
  ...
  return (
    <div className="space-y-8">
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center"><Users className="mr-2 h-5 w-5" /> Skill Tagging & Course Discovery</CardTitle>
        ...
        <CardContent className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          {skillToTag && coursesForSelectedSkill.length > 0 && (
            <div className="md:col-span-2 mt-4">
              <h4 className="text-lg font-semibold mb-2">Courses related to "{skills.find(s=>s.id === skillToTag)?.name}":</h4>
              <ul className="space-y-2">
                {coursesForSelectedSkill.map(course => (
                  <li key={course.id} className="p-3 border rounded-md text-sm">
           ...
           {skillToTag && coursesForSelectedSkill.length === 0 && (
            <p className="md:col-span-2 text-muted-foreground mt-2">No specific courses found for "{skills.find(s=>s.id === skillToTag)?.name}".</p>
           )}
        </CardContent>
      </Card>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Get course recommendations for specific employee groups.</CardDescription>
        </CardHeader>
        <CardContent>
          <div className="flex space-x-4 mb-4">
            <Button onClick={() => generateRecommendations('new_joiner')}>For New Joiners</Button>
            <Button onClick={() => generateRecommendations('cybersecurity_team')}>For Cybersecurity Team</Button>
          </div>
          {recommendationTarget && recommendedLearningPaths.length > 0 && (
            <div>
              <h4 className="text-lg font-semibold mb-2">Recommended for {recommendationTarget === 'new_joiner' ? 'New Joiners' : 'Cybersecurity Team'}:</h4>
              <ul className="space-y-2">
                {recommendedLearningPaths.map(course => (
                  <li key={course.id} className="p-3 border rounded-md text-sm">
                    <strong>{course.title}</strong> ({course.provider})
                    <p className="text-xs text-muted-foreground">Skills: {course.relatedSkills.map(id => skills.find(s=>s.id===id)?.name).join(', ')}</p>
                  </li>
                ))}
              </ul>
            </div>
          ...
          {recommendationTarget && recommendedLearningPaths.length === 0 && (
            <p className="text-muted-foreground">No specific recommendations found for this group.</p>
          )}
        </CardContent>
      </Card>
    </div>
...
// This component should be wrapped in PlatformLayout in its page.tsx
// For example: /app/platform/job-roles/page.tsx
// import PlatformLayout from "@/components/platform/PlatformLayout";
// import JobRoleManager from "@/components/platform/job-roles/JobRoleManager";
// const JobRolesPage = () => <PlatformLayout title="Job Roles & Recommendations"><JobRoleManager /></PlatformLayout>;
